...
exports.processEmailAnalysis = processEmailAnalysis;
...
//   }
// }
const analysis_service_js_1 = require("./analysis-service.js");
const action_service_js_1 = require("./action-service.js");
async function processEmailAnalysis(subject, body) {
    const startTime = Date.now();
    try {
        // First analyze the email
        const analysis = await (0, analysis_service_js_1.analyzeEmail)(subject, body);
        // If human review is required, we can skip action planning or adjust it
        const nextActionResult = analysis.requires_human_review
            ...
            : await (0, action_service_js_1.planNextAction)(analysis.classification, analysis.structured_data);
        const duration = Date.now() - startTime;
        return {
            ...analysis,
