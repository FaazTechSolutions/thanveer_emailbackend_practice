...
async function analyzeEmail(subject, body) {
    const emailText = (0, text_utils_js_1.formatEmailText)(subject, body);
    const analysisPrompt = (0, text_utils_js_1.formatPrompt)(ANALYSIS_PROMPT, emailText);
    try {
        // First get the main analysis
        const analysisResult = await (0, ai_1.generateText)({
            model: openrouter(MODEL),
            prompt: analysisPrompt,
            temperature: 0.1,
        });
        const parsedAnalysis = (0, json_utils_js_1.safeJsonParse)(analysisResult.text);
        if (!parsedAnalysis)
        ...
        if (!summary || parsedAnalysis.confidence_scores.overall < confidenceThreshold) {
            const summaryPrompt = (0, text_utils_js_1.formatPrompt)(SUMMARY_PROMPT, emailText);
        ...
        return {
            classification: (0, json_utils_js_1.validateConfidence)(parsedAnalysis.classification || {}),
            structured_data: {
                ...(parsedAnalysis.structured_data || {}),
                confidence: parsedAnalysis.structured_data?.confidence || 0.7
            },
            action_items: (parsedAnalysis.action_items || []).map(json_utils_js_1.validateConfidence),
 ...
 */
function generateFallbackSummary(analysis, body) {
 ...
 */
function checkForHumanReview(parsedAnalysis) {
    const reasons = [];
 ...
 */
function getFallbackAnalysis(subject, body) {
    const firstSentence = body.split(/[.!?]/)[0] || body.substring(0, 100);
    return {
        classification: {
            category: "other",
