...
async function bootstrap() {
    ...
    // ===========================================================
    router.get("/process-multiple-mails", async (req, res) => {
        const page = Number(req.query.page) || 1;
        const size = Number(req.query.size) || 1;
        const specificReqId = req.query.req_id || undefined;
        console.log(`
=============================================================
...
=============================================================
    `);
        try {
            ...
            for (const email of emails) {
                ...
                try {
                    const processedEmail = await (0, process1emai_js_1.default)(email, specificReqId);
    ...
    // ===========================================================
    router.get("/process-single-mail", async (req, res) => {
    ...
    // ===========================================================
    router.get("/get-mails-from-reprocess", async (req, res) => {
        const reqId = String(req.query.req_id || "").trim();
        if (!reqId)
    ...
    // ===========================================================
    router.get("/get-mail-from-cleaned", async (req, res) => {
        const reqId = String(req.query.req_id || "").trim();
        if (!reqId)
            return res.status(400).json({ success: false, error: "req_id required" });
        try {
